...
- **Map literal** as a call argument needs its own key-value collection
  ...
  a map-typed parameter instead, which is the value crossing without the
  literal. A library taking the table as `Map<string, V>` rather than a
  plain object would also need a conversion at the boundary
  (`new Map(Object.entries(..))`), which no spelling asks for today.
