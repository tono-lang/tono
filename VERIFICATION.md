...
| `ext` bindings against the real library, at the `.tono` | `tono check` compiles one probe line per binding with the target toolchain inside the consumer tree, beside the SDK's own type declarations generated in memory by the `tono gen` pipeline (so a binding naming a generated type is graded too), and maps each rejection to the binding's span; a library with no type source is reported, never silently skipped | fails the check on a divergence; the bench runs it on every row (`check-red`) | `backend/src/codegen/verify.rs`, `targets/{go,typescript}/entry/verify.rs`, `frontend/lib/syntax/ext_sites.ml`, `cli/tests/check.rs` |
| `ext` bindings in the editor (on save) | the language server runs `tono check --json --only <ext>=<lang>` per dirty pair and publishes its lines; the bench drives the server on every `check-red` row and diffs its verdict against the command's, line for line; the JSON form round-trips to the text form in a unit test | breaks build on a divergence between editor and command (`editor-red`) | `lsp/binding_runner.ml`, `lsp/binding_check.ml`, `lsp/test/lsp_check_driver.ml`, `scripts/check-ffi-bench.sh`, `cli/src/check.rs` |
