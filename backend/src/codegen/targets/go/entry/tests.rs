...
fn the_resolution_follows_the_declared_chains() {
    ...
    assert!(serde.contains("s.ClientKey = tono.StrUpperSnake(strings.TrimSpace(s.ClientName))"));
    ...
    assert!(serde.contains("ms, err := tono.DurationMs(string(s.Timeout))"));
...
fn transforms_apply_to_chain_and_match_resolved_values() {
    ...
    assert!(serde.contains("s.Team = tono.StrSnake(s.Team)"));
...
fn the_matrix_module_exercises_every_resolution_idiom() {
    ...
    assert!(serde.contains("tono.StrUpperSnake(tono.StrPascal(tono.StrKebab(tono.StrSnake(strings.ToUpper(strings.ToLower(strings.TrimSpace("));
