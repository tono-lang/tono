...
pub fn emit(module: &Module, config: &CasingConfig) -> EntryEmission {
    ...
    for entry in &entries {
        ...
        for resolver in ext_resolver::resolver_decls(entry, module, config, multi) {
            if !own_libs.contains(&resolver.lib) {
                own_libs.push(resolver.lib.clone());
            }
            let lib = ext.iter_mut().find(|(name, _)| *name == resolver.lib);
            match lib {
        ...
        // Every foreign construction the entry's own tail resolves calls a
        // function that lives in that library's own group (`ext_resolver::
        // resolver_decls`); a plain call site names it bare (a Rust text
        // symbol carries no ref of its own the way an entry's other cross-
        // group calls do), so this glob brings each one in, the same
        // technique the module's own types glob below already uses.
        for lib in &own_libs {
            decls.insert(
                0,
                crate::codegen::targets::rust::emit::types_glob_use(
                    &crate::codegen::group::Group::ext(&module.name, lib),
                ),
            );
        }
...
pub(super) fn entry_text(module: &Module, config: &CasingConfig) -> String {
    ...
    for (_, ext_decls) in emission.ext {
        decls.extend(ext_decls);
    }
