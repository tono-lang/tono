...
(* The receiver/method/arity/argument rules are shared with a field's own
   "= .field.method(args)" value source and live in [Check_handle_call]. *)
let check_op_impl ctx (fields : Ast.member list) (op : Ast.decl) :
    Diagnostic.t list =
  match op.Ast.dkind with
  | Ast.DOp { oimpl = Some oi; _ } ->
      let pname, pty = op_param op in
      snd (Check_handle_call.check ctx ~fields ~pname ~pty ~what:"'impl'" oi)
  | _ -> []
