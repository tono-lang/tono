...
const IR: &str = r#"{"tono_ir_version":28,"modules":[{"name":"demo","shapes":[{"id":"demo#Charge","kind":"structure","params":[],"members":[{"name":"amount","required":true,"target":{"prim":"i64"},"constraints":[],"traits":[]}],"operations":[]}],"operations":[]}]}"#;

/// A contract extension with no conformance reference: the generator must refuse
/// to emit (AC-4). Everything else is well-formed.
const IR_UNCONFORMANT_CONTRACT: &str = r#"{"tono_ir_version":28,"modules":[{"name":"demo","shapes":[],"operations":[],"extensions":[{"name":"sign","kind":"contract","bindings":{"ts":"ext/ts/s.ts#sign"},"signature":{"input":{"prim":"string"},"output":{"prim":"string"}}}]}]}"#;
...
const DOTTED_IR: &str = r#"{"tono_ir_version":28,"modules":[{"name":"payments.common","shapes":[{"id":"payments.common#Money","kind":"structure","params":[],"members":[{"name":"amount","required":true,"target":{"prim":"i64"},"constraints":[],"traits":[]}],"operations":[]}],"operations":[]}]}"#;
...
const TWO_MODULE_IR: &str = r#"{"tono_ir_version":28,"modules":[{"name":"payments.common","shapes":[{"id":"payments.common#Money","kind":"structure","params":[],"members":[{"name":"amount","required":true,"target":{"prim":"i64"},"constraints":[],"traits":[]}],"operations":[]}],"operations":[]},{"name":"payments.charge","shapes":[{"id":"payments.charge#Charge","kind":"structure","params":[],"members":[{"name":"total","required":true,"target":{"ref":"payments.common#Money","args":[]},"constraints":[],"traits":[]}],"operations":[]}],"operations":[]}]}"#;
...
const CASING_IR: &str = r#"{"tono_ir_version":28,"modules":[{"name":"demo","shapes":[{"id":"demo#Event","kind":"structure","params":[],"members":[{"name":"created_at","required":true,"target":{"prim":"string"},"constraints":[],"traits":[]}],"operations":[]}],"operations":[]}]}"#;
...
fn manifest_drives_enabled_targets_and_out_dirs() {
    ...
    assert!(base.join("gen/rust/src/demo/types.rs").is_file());
...
fn manifest_module_mapping_flat_flattens_the_layout() {
    ...
    assert!(base.join("r/src/payments_common/types.rs").is_file());
    assert!(!base.join("r/src/payments/common/types.rs").exists());
    let _ = std::fs::remove_dir_all(&base);
}
...
fn manifest_module_remap_rewrites_the_prefix() {
    ...
    let out = gen_stdin_in(&base, &["--config", manifest.to_str().unwrap()], DOTTED_IR);
    ok_or_stderr(&out);
    assert!(base.join("r/src/billing/common/types.rs").is_file());
    let _ = std::fs::remove_dir_all(&base);
}
...
fn clean_removes_generated_files_this_run_did_not_produce() {
    ...
    let stale_dir = out.join("rust").join("src").join("gone");
    ...
    assert!(
        out.join("rust/src/demo/types.rs").is_file(),
...
fn gen_leaves_stale_output_alone_without_clean() {
    ...
    assert!(gen_via_stdin(&out, "rust"));
    let stale = out.join("rust").join("src").join("stale.rs");
    std::fs::write(
        &stale,
        "// Code generated by tono. DO NOT EDIT.\n\npub struct S;\n",
    )
